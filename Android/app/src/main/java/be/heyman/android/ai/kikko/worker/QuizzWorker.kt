...
class QuizWorker(val appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    ...
    companion object {
        const val TAG = "QuizWorker"
    }

    override suspend fun doWork(): Result {
        Log.d(TAG, "Le QuizWorker démarre sa tâche.")
        ...
        val grainToProcess = pollenGrainDao.getByStatus(PollenStatus.PENDING_QUIZ).firstOrNull()
        if (grainToProcess == null) {
            Log.d(TAG, "Aucun pollen en attente de quiz trouvé. Tâche terminée.")
        ...
        if (cardId == null) {
            Log.e(TAG, "Erreur critique: Le grain ${grainToProcess.id} est PENDING_QUIZ mais n'a pas de forgedCardId.")
            pollenGrainDao.updateStatus(grainToProcess.id, PollenStatus.ERROR)
            return Result.failure()
        }

        val cardToUpdate = cardDao.getCardById(cardId)
        if (cardToUpdate == null || cardToUpdate.description.isNullOrBlank() || cardToUpdate.stats == null) {
            Log.e(TAG, "Erreur critique: Carte ID $cardId ou ses données (description/stats) sont manquantes pour le quiz.")
            pollenGrainDao.updateStatus(grainToProcess.id, PollenStatus.ERROR)
            return Result.failure()
        }

        setForeground(createForegroundInfo("La Reine '${selectedQueenName}' forge le quiz..."))

        try {
            Log.i(TAG, "Génération du quiz pour la carte ID: $cardId (Pollen ID: ${grainToProcess.id})")
            ...
            val prompt = PromptGenerator.generateQuizPrompt(cardToUpdate.specificName, cardToUpdate.description, cardToUpdate.stats, Locale.getDefault())
            ...
            Log.i(TAG, "Quiz ajouté à la carte ID: $cardId")

            pollenGrainDao.updateStatus(grainToProcess.id, PollenStatus.PENDING_TRANSLATION)
            Log.i(TAG, "Le grain ${grainToProcess.id} est maintenant en attente de traduction. Tâche réussie.")
            return Result.success()

        } catch (e: Exception) {
            Log.e(TAG, "Échec de la génération du quiz pour la carte ID: $cardId", e)
            pollenGrainDao.updateStatus(grainToProcess.id, PollenStatus.ERROR)
            return Result.failure()
        } finally {
            llmHelper.cleanUp()
    ...
    override suspend fun getForegroundInfo(): ForegroundInfo {
        return createForegroundInfo("La Forge de la Ruche prépare le quiz...")
    }
